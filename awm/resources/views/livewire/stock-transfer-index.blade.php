<div>
    <x-page-header title="Stock Transfers" subtitle="Transfer stock between racks">
    ...
    <x-data-table empty-text="No transfer records found.">
        <x-slot:header>
        ...
        @forelse ($items as $movement)
            <tr wire:key="st-{{ $movement->id }}">
                ...
                <td class="whitespace-nowrap px-4 py-3 text-sm font-semibold text-blue-600">{{ $movement->quantity }}</td>
                <td class="whitespace-nowrap px-4 py-3 text-sm text-gray-500">{{ $movement->notes ?? '—' }}</td>
            </tr>
        @empty
            <tr><td colspan="6" class="px-4 py-12 text-center text-sm text-gray-500">No transfer records found.</td></tr>
        @endforelse
    </x-data-table>

    <div class="mt-4">{{ $items->links() }}</div>

    <x-modal name="stock-transfer-modal" wire:model.live="showModal" max-width="lg">
        <h2 class="mb-4 text-lg font-semibold text-gray-900">Stock Transfer</h2>
        <form wire:submit="save" class="space-y-4">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <x-select label="Source Rack" wire:model="sourceRackId" name="sourceRackId" :options="$this->getSourceRackOptions()" placeholder="Select source..." required :error="$errors->first('sourceRackId')" />
                <div>
                    <x-select label="Destination Rack" wire:model="destRackId" name="destRackId" :options="$this->getDestRackOptions()" placeholder="Select destination..." required :error="$errors->first('destRackId')" />
                    @if ($sourceAvailable > 0)
                        <p class="mt-1 text-xs text-gray-500">Available at source: {{ $sourceAvailable }} units</p>
                    @endif
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3 pt-4">
                ...
                <x-button variant="primary" type="submit">Confirm Transfer</x-button>
            </div>
        </form>
    </x-modal>
</div>