<div>
    <x-page-header title="Technicians" subtitle="Manage workshop technicians">
    ...
    <div class="mb-4">
        <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search technicians..." class="block w-full max-w-sm rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
    </div>

    <x-data-table empty-text="No technicians found.">
        <x-slot:header>
            ...
            <tr wire:key="tech-{{ $tech->id }}">
                ...
                <td class="whitespace-nowrap px-4 py-3 text-sm">
                    <div class="flex items-center gap-2">
                        <x-button variant="secondary" size="sm" wire:click="openEditModal({{ $tech->id }})">Edit</x-button>
                        <x-button variant="danger" size="sm" wire:click="confirmDelete({{ $tech->id }})">Delete</x-button>
                    </div>
                </td>
            </tr>
        @empty
            <tr><td colspan="4" class="px-4 py-12 text-center text-sm text-gray-500">No technicians found.</td></tr>
        @endforelse
    </x-data-table>

    <div class="mt-4">{{ $items->links() }}</div>

    <x-modal name="technician-modal" wire:model.live="showModal">
        <h2 class="mb-4 text-lg font-semibold text-gray-900">{{ $editingId ? 'Edit' : 'Create' }} Technician</h2>
        <form wire:submit="save" class="space-y-4">
            <x-input label="Full Name" wire:model="name" name="name" placeholder="e.g. Budi Santoso" required :error="$errors->first('name')" />
